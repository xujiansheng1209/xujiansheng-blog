<template><div><p>在学习 NIO 之前，需要先了解一下计算机 I/O 模型的基础理论知识。还不了解的话，可以参考我写的这篇文章：<a href="https://javaguide.cn/java/io/io-model.html" target="_blank" rel="noopener noreferrer">Java IO 模型详解<ExternalLinkIcon/></a>。</p>
<h2 id="nio-简介" tabindex="-1"><a class="header-anchor" href="#nio-简介"><span>NIO 简介</span></a></h2>
...
<p>下图是 BIO、NIO 和 AIO 处理客户端请求的简单对比图（关于 AIO 的介绍，可以看我写的这篇文章：<a href="https://javaguide.cn/java/io/io-model.html" target="_blank" rel="noopener noreferrer">Java IO 模型详解<ExternalLinkIcon/></a>，不是重点，了解即可）。</p>
...
<h2 id="nio-核心组件" tabindex="-1"><a class="header-anchor" href="#nio-核心组件"><span>NIO 核心组件</span></a></h2>
...
<p>下面详细介绍一下这三个组件。</p>
<h3 id="buffer-缓冲区" tabindex="-1"><a class="header-anchor" href="#buffer-缓冲区"><span>Buffer（缓冲区）</span></a></h3>
...
<p><code v-pre>Buffer</code> 的子类如下图所示。其中，最常用的是 <code v-pre>ByteBuffer</code>，它可以用来存储和操作字节数据。</p>
...
<p>你可以将 Buffer 理解为一个数组，<code v-pre>IntBuffer</code>、<code v-pre>FloatBuffer</code>、<code v-pre>CharBuffer</code> 等分别对应 <code v-pre>int[]</code>、<code v-pre>float[]</code>、<code v-pre>char[]</code> 等。</p>
<p>为了更清晰地认识缓冲区，我们来简单看看<code v-pre>Buffer</code> 类中定义的四个成员变量：</p>
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">abstract</span> <span class="token keyword">class</span> <span class="token class-name">Buffer</span> <span class="token punctuation">{</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这四个成员变量的具体含义如下：</p>
<ol>
<li>容量（<code v-pre>capacity</code>）：<code v-pre>Buffer</code>可以存储的最大数据量，<code v-pre>Buffer</code>创建时设置且不可改变；</li>
<li>界限（<code v-pre>limit</code>）：<code v-pre>Buffer</code> 中可以读/写数据的边界。写模式下，<code v-pre>limit</code> 代表最多能写入的数据，一般等于 <code v-pre>capacity</code>（可以通过<code v-pre>limit(int newLimit)</code>方法设置）；读模式下，<code v-pre>limit</code> 等于 Buffer 中实际写入的数据大小。</li>
<li>位置（<code v-pre>position</code>）：下一个可以被读写的数据的位置（索引）。从写操作模式到读操作模式切换的时候（flip），<code v-pre>position</code> 都会归零，这样就可以从头开始读写了。</li>
<li>标记（<code v-pre>mark</code>）：<code v-pre>Buffer</code>允许将位置直接定位到该标记处，这是一个可选属性；</li>
</ol>
<p>并且，上述变量满足如下的关系：<strong>0 &lt;= mark &lt;= position &lt;= limit &lt;= capacity</strong> 。</p>
<p>另外，Buffer 有读模式和写模式这两种模式，分别用于从 Buffer 中读取数据或者向 Buffer 中写入数据。Buffer 被创建之后默认是写模式，调用 <code v-pre>flip()</code> 可以切换到读模式。如果要再次切换回写模式，可以调用 <code v-pre>clear()</code> 或者 <code v-pre>compact()</code> 方法。</p>
...
<p><code v-pre>Buffer</code> 对象不能通过 <code v-pre>new</code> 调用构造方法创建对象 ，只能通过静态方法实例化 <code v-pre>Buffer</code>。</p>
<p>这里以 <code v-pre>ByteBuffer</code>为例进行介绍：</p>
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code><span class="token comment">// 分配堆内存</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>Buffer 最核心的两个方法：</p>
<ol>
<li><code v-pre>get</code> : 读取缓冲区的数据</li>
<li><code v-pre>put</code> ：向缓冲区写入数据</li>
</ol>
<p>除上述两个方法之外，其他的重要方法：</p>
<ul>
<li><code v-pre>flip</code> ：将缓冲区从写模式切换到读模式，它会将 <code v-pre>limit</code> 的值设置为当前 <code v-pre>position</code> 的值，将 <code v-pre>position</code> 的值设置为 0。</li>
<li><code v-pre>clear</code>: 清空缓冲区，将缓冲区从读模式切换到写模式，并将 <code v-pre>position</code> 的值设置为 0，将 <code v-pre>limit</code> 的值设置为 <code v-pre>capacity</code> 的值。</li>
<li>……</li>
</ul>
<p>Buffer 中数据变化的过程：</p>
...
<div class="language-bash line-numbers-mode" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code>初始状态：
...
capacity: <span class="token number">8</span>, limit: <span class="token number">8</span>, position: <span class="token number">0</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>为了帮助理解，我绘制了一张图片展示 <code v-pre>capacity</code>、<code v-pre>limit</code>和<code v-pre>position</code>每一阶段的变化。</p>
...
<h3 id="channel-通道" tabindex="-1"><a class="header-anchor" href="#channel-通道"><span>Channel（通道）</span></a></h3>
<p>Channel 是一个通道，它建立了与数据源（如文件、网络套接字等）之间的连接。我们可以利用它来读取和写入数据，就像打开了一条自来水管，让数据在 Channel 中自由流动。</p>
<p>BIO 中的流是单向的，分为各种 <code v-pre>InputStream</code>（输入流）和 <code v-pre>OutputStream</code>（输出流），数据只是在一个方向上传输。通道与流的不同之处在于通道是双向的，它可以用于读、写或者同时用于读写。</p>
...
<p><code v-pre>Channel</code> 的子类如下图所示。</p>
...
<ul>
<li><code v-pre>FileChannel</code>：文件访问通道；</li>
<li><code v-pre>SocketChannel</code>、<code v-pre>ServerSocketChannel</code>：TCP 通信通道；</li>
<li><code v-pre>DatagramChannel</code>：UDP 通信通道；</li>
</ul>
...
<ol>
<li><code v-pre>read</code> ：读取数据并写入到 Buffer 中。</li>
<li><code v-pre>write</code> ：将 Buffer 中的数据写入到 Channel 中。</li>
</ol>
<p>这里我们以 <code v-pre>FileChannel</code> 为例演示一下是读取文件数据的。</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="selector-选择器" tabindex="-1"><a class="header-anchor" href="#selector-选择器"><span>Selector（选择器）</span></a></h3>
...
<p>一个多路复用器 Selector 可以同时轮询多个 Channel，由于 JDK 使用了 <code v-pre>epoll()</code> 代替传统的 <code v-pre>select</code> 实现，所以它并没有最大连接句柄 <code v-pre>1024/2048</code> 的限制。这也就意味着只需要一个线程负责 Selector 的轮询，就可以接入成千上万的客户端。</p>
<p>Selector 可以监听以下四种事件类型：</p>
<ol>
<li><code v-pre>SelectionKey.OP_ACCEPT</code>：表示通道接受连接的事件，这通常用于 <code v-pre>ServerSocketChannel</code>。</li>
<li><code v-pre>SelectionKey.OP_CONNECT</code>：表示通道完成连接的事件，这通常用于 <code v-pre>SocketChannel</code>。</li>
<li><code v-pre>SelectionKey.OP_READ</code>：表示通道准备好进行读取的事件，即有数据可读。</li>
<li><code v-pre>SelectionKey.OP_WRITE</code>：表示通道准备好进行写入的事件，即可以写入数据。</li>
</ol>
<p><code v-pre>Selector</code>是抽象类，可以通过调用此类的 <code v-pre>open()</code> 静态方法来创建 Selector 实例。Selector 可以同时监控多个 <code v-pre>SelectableChannel</code> 的 <code v-pre>IO</code> 状况，是非阻塞 <code v-pre>IO</code> 的核心。</p>
<p>一个 Selector 实例有三个 <code v-pre>SelectionKey</code> 集合：</p>
<ol>
<li>所有的 <code v-pre>SelectionKey</code> 集合：代表了注册在该 Selector 上的 <code v-pre>Channel</code>，这个集合可以通过 <code v-pre>keys()</code> 方法返回。</li>
<li>被选择的 <code v-pre>SelectionKey</code> 集合：代表了所有可通过 <code v-pre>select()</code> 方法获取的、需要进行 <code v-pre>IO</code> 处理的 Channel，这个集合可以通过 <code v-pre>selectedKeys()</code> 返回。</li>
<li>被取消的 <code v-pre>SelectionKey</code> 集合：代表了所有被取消注册关系的 <code v-pre>Channel</code>，在下一次执行 <code v-pre>select()</code> 方法时，这些 <code v-pre>Channel</code> 对应的 <code v-pre>SelectionKey</code> 会被彻底删除，程序通常无须直接访问该集合，也没有暴露访问的方法。</li>
</ol>
<p>简单演示一下如何遍历被选择的 <code v-pre>SelectionKey</code> 集合并进行处理：</p>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>Selector 还提供了一系列和 <code v-pre>select()</code> 相关的方法：</p>
<ul>
<li><code v-pre>int select()</code>：监控所有注册的 <code v-pre>Channel</code>，当它们中间有需要处理的 <code v-pre>IO</code> 操作时，该方法返回，并将对应的 <code v-pre>SelectionKey</code> 加入被选择的 <code v-pre>SelectionKey</code> 集合中，该方法返回这些 <code v-pre>Channel</code> 的数量。</li>
<li><code v-pre>int select(long timeout)</code>：可以设置超时时长的 <code v-pre>select()</code> 操作。</li>
<li><code v-pre>int selectNow()</code>：执行一个立即返回的 <code v-pre>select()</code> 操作，相对于无参数的 <code v-pre>select()</code> 方法而言，该方法不会阻塞线程。</li>
<li><code v-pre>Selector wakeup()</code>：使一个还未返回的 <code v-pre>select()</code> 方法立刻返回。</li>
<li>……</li>
</ul>
...
<h2 id="nio-零拷贝" tabindex="-1"><a class="header-anchor" href="#nio-零拷贝"><span>NIO 零拷贝</span></a></h2>
<p>零拷贝是提升 IO 操作性能的一个常用手段，像 ActiveMQ、Kafka 、RocketMQ、QMQ、Netty 等顶级开源项目都用到了零拷贝。</p>
<p>零拷贝是指计算机执行 IO 操作时，CPU 不需要将数据从一个存储区域复制到另一个存储区域，从而可以减少上下文切换以及 CPU 的拷贝时间。也就是说，零拷贝主主要解决操作系统在处理 I/O 操作时频繁复制数据的问题。零拷贝的常见实现技术有： <code v-pre>mmap+write</code>、<code v-pre>sendfile</code>和 <code v-pre>sendfile + DMA gather copy</code> 。</p>
...
<p>Java 对零拷贝的支持：</p>
<ul>
<li><code v-pre>MappedByteBuffer</code> 是 NIO 基于内存映射（<code v-pre>mmap</code>）这种零拷⻉⽅式的提供的⼀种实现，底层实际是调用了 Linux 内核的 <code v-pre>mmap</code> 系统调用。它可以将一个文件或者文件的一部分映射到内存中，形成一个虚拟内存文件，这样就可以直接操作内存中的数据，而不需要通过系统调用来读写文件。</li>
<li><code v-pre>FileChannel</code> 的<code v-pre>transferTo()/transferFrom()</code>是 NIO 基于发送文件（<code v-pre>sendfile</code>）这种零拷贝方式的提供的一种实现，底层实际是调用了 Linux 内核的 <code v-pre>sendfile</code>系统调用。它可以直接将文件数据从磁盘发送到网络，而不需要经过用户空间的缓冲区。关于<code v-pre>FileChannel</code>的用法可以看看这篇文章：<a href="https://www.cnblogs.com/robothy/p/14235598.html" target="_blank" rel="noopener noreferrer">Java NIO 文件通道 FileChannel 用法<ExternalLinkIcon/></a>。</li>
</ul>
<p>代码示例：</p>
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code><span class="token keyword">private</span> <span class="token keyword">void</span> <span class="token function">loadFileIntoMemory</span><span class="token punctuation">(</span><span class="token class-name">File</span> xmlFile<span class="token punctuation">)</span> <span class="token keyword">throws</span> <span class="token class-name">IOException</span> <span class="token punctuation">{</span>
  <span class="token class-name">FileInputStream</span> fis <span class="token operator">=</span> <span class="token keyword">new</span> <span class="token class-name">FileInputStream</span><span class="token punctuation">(</span>xmlFile<span class="token punctuation">)</span><span class="token punctuation">;</span>
  <span class="token comment">// 创建 FileChannel 对象</span>
  ...
  <span class="token comment">// FileChannel.map() 将文件映射到直接内存并返回 MappedByteBuffer 对象</span>
  <span class="token class-name">MappedByteBuffer</span> mmb <span class="token operator">=</span> fc<span class="token punctuation">.</span><span class="token function">map</span><span class="token punctuation">(</span><span class="token class-name">FileChannel<span class="token punctuation">.</span>MapMode</span><span class="token punctuation">.</span><span class="token constant">READ_ONLY</span><span class="token punctuation">,</span> <span class="token number">0</span><span class="token punctuation">,</span> fc<span class="token punctuation">.</span><span class="token function">size</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
  xmlFileBuffer <span class="token operator">=</span> <span class="token keyword">new</span> <span class="token keyword">byte</span><span class="token punctuation">[</span><span class="token punctuation">(</span><span class="token keyword">int</span><span class="token punctuation">)</span>fc<span class="token punctuation">.</span><span class="token function">size</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">]</span><span class="token punctuation">;</span>
  mmb<span class="token punctuation">.</span><span class="token function">get</span><span class="token punctuation">(</span>xmlFileBuffer<span class="token punctuation">)</span><span class="token punctuation">;</span>
...
<ul>
<li>
<p>Java NIO 浅析：<a href="https://tech.meituan.com/2016/11/04/nio.html" target="_blank" rel="noopener noreferrer">https://tech.meituan.com/2016/11/04/nio.html<ExternalLinkIcon/></a></p>
</li>
<li>
<p>面试官：Java NIO 了解？<a href="https://mp.weixin.qq.com/s/mZobf-U8OSYQfHfYBEB6KA" target="_blank" rel="noopener noreferrer">https://mp.weixin.qq.com/s/mZobf-U8OSYQfHfYBEB6KA<ExternalLinkIcon/></a></p>
</li>
<li>
<p>Java NIO：Buffer、Channel 和 Selector：<a href="https://www.javadoop.com/post/java-nio" target="_blank" rel="noopener noreferrer">https://www.javadoop.com/post/java-nio<ExternalLinkIcon/></a></p>
</li>
</ul>
</div></template>
