<template><div><h2 id="jmm-java-内存模型" tabindex="-1"><a class="header-anchor" href="#jmm-java-内存模型"><span>JMM(Java 内存模型)</span></a></h2>
<p>JMM（Java 内存模型）相关的问题比较多，也比较重要，于是我单独抽了一篇文章来总结 JMM 相关的知识点和问题：<RouteLink to="/java-core/concurrent/jmm.html">JMM（Java 内存模型）详解</RouteLink> 。</p>
<h2 id="volatile-关键字" tabindex="-1"><a class="header-anchor" href="#volatile-关键字"><span>volatile 关键字</span></a></h2>
<h3 id="如何保证变量的可见性" tabindex="-1"><a class="header-anchor" href="#如何保证变量的可见性"><span>如何保证变量的可见性？</span></a></h3>
...
<p><code v-pre>volatile</code> 关键字其实并非是 Java 语言特有的，在 C 语言里也有，它最原始的意义就是禁用 CPU 缓存。如果我们将一个变量使用 <code v-pre>volatile</code> 修饰，这就指示 编译器，这个变量是共享且不稳定的，每次使用它都到主存中进行读取。</p>
<p><code v-pre>volatile</code> 关键字能保证数据的可见性，但不能保证数据的原子性。<code v-pre>synchronized</code> 关键字两者都能保证。</p>
<h3 id="如何禁止指令重排序" tabindex="-1"><a class="header-anchor" href="#如何禁止指令重排序"><span>如何禁止指令重排序？</span></a></h3>
<p><strong>在 Java 中，<code v-pre>volatile</code> 关键字除了可以保证变量的可见性，还有一个重要的作用就是防止 JVM 的指令重排序。</strong> 如果我们将变量声明为 <strong><code v-pre>volatile</code></strong> ，在对这个变量进行读写操作的时候，会通过插入特定的 <strong>内存屏障</strong> 的方式来禁止指令重排序。</p>
<p>在 Java 中，<code v-pre>Unsafe</code> 类提供了三个开箱即用的内存屏障相关的方法，屏蔽了操作系统底层的差异：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>理论上来说，你通过这个三个方法也可以实现和<code v-pre>volatile</code>禁止重排序一样的效果，只是会麻烦一些。</p>
<p>下面我以一个常见的面试题为例讲解一下 <code v-pre>volatile</code> 关键字禁止指令重排序的效果。</p>
...
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">Singleton</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>uniqueInstance</code> 采用 <code v-pre>volatile</code> 关键字修饰也是很有必要的， <code v-pre>uniqueInstance = new Singleton();</code> 这段代码其实是分为三步执行：</p>
<ol>
...
<h3 id="volatile-可以保证原子性么" tabindex="-1"><a class="header-anchor" href="#volatile-可以保证原子性么"><span>volatile 可以保证原子性么？</span></a></h3>
<p><strong><code v-pre>volatile</code> 关键字能保证变量的可见性，但不能保证对变量的操作是原子性的。</strong></p>
<p>我们通过下面的代码即可证明：</p>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">VolatileAtomicityDemo</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>正常情况下，运行上面的代码理应输出 <code v-pre>2500</code>。但你真正运行了上面的代码之后，你会发现每次输出结果都小于 <code v-pre>2500</code>。</p>
<p>为什么会出现这种情况呢？不是说好了，<code v-pre>volatile</code> 可以保证变量的可见性嘛！</p>
<p>也就是说，如果 <code v-pre>volatile</code> 能保证 <code v-pre>inc++</code> 操作的原子性的话。每个线程中对 <code v-pre>inc</code> 变量自增完之后，其他线程可以立即看到修改后的值。5 个线程分别进行了 500 次操作，那么最终 inc 的值应该是 5*500=2500。</p>
<p>很多人会误认为自增操作 <code v-pre>inc++</code> 是原子性的，实际上，<code v-pre>inc++</code> 其实是一个复合操作，包括三步：</p>
<ol>
<li>读取 inc 的值。</li>
<li>对 inc 加 1。</li>
<li>将 inc 的值写回内存。</li>
</ol>
<p><code v-pre>volatile</code> 是无法保证这三个操作是具有原子性的，有可能导致下面这种情况出现：</p>
<ol>
<li>线程 1 对 <code v-pre>inc</code> 进行读取操作之后，还未对其进行修改。线程 2 又读取了 <code v-pre>inc</code>的值并对其进行修改（+1），再将<code v-pre>inc</code> 的值写回内存。</li>
<li>线程 2 操作完毕后，线程 1 对 <code v-pre>inc</code>的值进行修改（+1），再将<code v-pre>inc</code> 的值写回内存。</li>
</ol>
<p>这也就导致两个线程分别对 <code v-pre>inc</code> 进行了一次自增操作后，<code v-pre>inc</code> 实际上只增加了 1。</p>
<p>其实，如果想要保证上面的代码运行正确也非常简单，利用 <code v-pre>synchronized</code>、<code v-pre>Lock</code>或者<code v-pre>AtomicInteger</code>都可以。</p>
<p>使用 <code v-pre>synchronized</code> 改进：</p>
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">synchronized</span> <span class="token keyword">void</span> <span class="token function">increase</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
    inc<span class="token operator">++</span><span class="token punctuation">;</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>使用 <code v-pre>AtomicInteger</code> 改进：</p>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>使用 <code v-pre>ReentrantLock</code> 改进：</p>
...
<span class="token keyword">public</span> <span class="token keyword">void</span> <span class="token function">increase</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="乐观锁和悲观锁" tabindex="-1"><a class="header-anchor" href="#乐观锁和悲观锁"><span>乐观锁和悲观锁</span></a></h2>
<h3 id="什么是悲观锁" tabindex="-1"><a class="header-anchor" href="#什么是悲观锁"><span>什么是悲观锁？</span></a></h3>
...
<p>像 Java 中<code v-pre>synchronized</code>和<code v-pre>ReentrantLock</code>等独占锁就是悲观锁思想的实现。</p>
...
<h3 id="什么是乐观锁" tabindex="-1"><a class="header-anchor" href="#什么是乐观锁"><span>什么是乐观锁？</span></a></h3>
<p>乐观锁总是假设最好的情况，认为共享资源每次被访问的时候不会出现问题，线程可以不停地执行，无需加锁也无需等待，只是在提交修改的时候去验证对应的资源（也就是数据）是否被其它线程修改了（具体方法可以使用版本号机制或 CAS 算法）。</p>
<p>在 Java 中<code v-pre>java.util.concurrent.atomic</code>包下面的原子变量类（比如<code v-pre>AtomicInteger</code>、<code v-pre>LongAdder</code>）就是使用了乐观锁的一种实现方式 <strong>CAS</strong> 实现的。<br>
<img src="https://oss.javaguide.cn/github/javaguide/java/JUC原子类概览-20230814005211968.png" alt="JUC原子类概览"></p>
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code><span class="token comment">// LongAdder 在高并发场景下会比 AtomicInteger 和 AtomicLong 的性能更好</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>高并发的场景下，乐观锁相比悲观锁来说，不存在锁竞争造成线程阻塞，也不会有死锁的问题，在性能上往往会更胜一筹。但是，如果冲突频繁发生（写占比非常多的情况），会频繁失败和重试，这样同样会非常影响性能，导致 CPU 飙升。</p>
<p>不过，大量失败重试的问题也是可以解决的，像我们前面提到的 <code v-pre>LongAdder</code>以空间换时间的方式就解决了这个问题。</p>
<p>理论上来说：</p>
<ul>
<li>悲观锁通常多用于写比较多的情况（多写场景，竞争激烈），这样可以避免频繁失败和重试影响性能，悲观锁的开销是固定的。不过，如果乐观锁解决了频繁失败和重试这个问题的话（比如<code v-pre>LongAdder</code>），也是可以考虑使用乐观锁的，要视实际情况而定。</li>
<li>乐观锁通常多用于写比较少的情况（多读场景，竞争较少），这样可以避免频繁加锁影响性能。不过，乐观锁主要针对的对象是单个共享变量（参考<code v-pre>java.util.concurrent.atomic</code>包下面的原子变量类）。</li>
</ul>
<h3 id="如何实现乐观锁" tabindex="-1"><a class="header-anchor" href="#如何实现乐观锁"><span>如何实现乐观锁？</span></a></h3>
<p>乐观锁一般会使用版本号机制或 CAS 算法实现，CAS 算法相对来说更多一些，这里需要格外注意。</p>
<h4 id="版本号机制" tabindex="-1"><a class="header-anchor" href="#版本号机制"><span>版本号机制</span></a></h4>
<p>一般是在数据表中加上一个数据版本号 <code v-pre>version</code> 字段，表示数据被修改的次数。当数据被修改时，<code v-pre>version</code> 值会加一。当线程 A 要更新数据值时，在读取数据的同时也会读取 <code v-pre>version</code> 值，在提交更新时，若刚才读取到的 version 值为当前数据库中的 <code v-pre>version</code> 值相等时才更新，否则重试更新操作，直到更新成功。</p>
<p><strong>举一个简单的例子</strong>：假设数据库中帐户信息表中有一个 version 字段，当前值为 1 ；而当前帐户余额字段（ <code v-pre>balance</code> ）为 $100 。</p>
<ol>
<li>操作员 A 此时将其读出（ <code v-pre>version</code>=1 ），并从其帐户余额中扣除 $50（ $100-$50 ）。</li>
<li>在操作员 A 操作的过程中，操作员 B 也读入此用户信息（ <code v-pre>version</code>=1 ），并从其帐户余额中扣除 $20 （ $100-$20 ）。</li>
<li>操作员 A 完成了修改工作，将数据版本号（ <code v-pre>version</code>=1 ），连同帐户扣除后余额（ <code v-pre>balance</code>=$50 ），提交至数据库更新，此时由于提交数据版本等于数据库记录当前版本，数据被更新，数据库记录 <code v-pre>version</code> 更新为 2 。</li>
<li>操作员 B 完成了操作，也将版本号（ <code v-pre>version</code>=1 ）试图向数据库提交数据（ <code v-pre>balance</code>=$80 ），但此时比对数据库记录版本时发现，操作员 B 提交的数据版本号为 1 ，数据库记录当前版本也为 2 ，不满足 “ 提交版本必须等于当前版本才能执行更新 “ 的乐观锁策略，因此，操作员 B 的提交被驳回。</li>
</ol>
<p>这样就避免了操作员 B 用基于 <code v-pre>version</code>=1 的旧数据修改的结果覆盖操作员 A 的操作结果的可能。</p>
<h4 id="cas-算法" tabindex="-1"><a class="header-anchor" href="#cas-算法"><span>CAS 算法</span></a></h4>
...
<p><code v-pre>sun.misc</code>包下的<code v-pre>Unsafe</code>类提供了<code v-pre>compareAndSwapObject</code>、<code v-pre>compareAndSwapInt</code>、<code v-pre>compareAndSwapLong</code>方法来实现的对<code v-pre>Object</code>、<code v-pre>int</code>、<code v-pre>long</code>类型的 CAS 操作</p>
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code><span class="token doc-comment comment">/**
  *  CAS
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>关于 <code v-pre>Unsafe</code> 类的详细介绍可以看这篇文章：<a href="https://javaguide.cn/java/basis/unsafe.html" target="_blank" rel="noopener noreferrer">Java 魔法类 Unsafe 详解 - JavaGuide - 2022<ExternalLinkIcon/></a> 。</p>
<h3 id="cas-算法存在哪些问题" tabindex="-1"><a class="header-anchor" href="#cas-算法存在哪些问题"><span>CAS 算法存在哪些问题？</span></a></h3>
<p>ABA 问题是 CAS 算法最常见的问题。</p>
<h4 id="aba-问题" tabindex="-1"><a class="header-anchor" href="#aba-问题"><span>ABA 问题</span></a></h4>
...
<p>ABA 问题的解决思路是在变量前面追加上<strong>版本号或者时间戳</strong>。JDK 1.5 以后的 <code v-pre>AtomicStampedReference</code> 类就是用来解决 ABA 问题的，其中的 <code v-pre>compareAndSet()</code> 方法就是首先检查当前引用是否等于预期引用，并且当前标志是否等于预期标志，如果全部相等，则以原子方式将该引用和该标志的值设置为给定的更新值。</p>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="循环时间长开销大" tabindex="-1"><a class="header-anchor" href="#循环时间长开销大"><span>循环时间长开销大</span></a></h4>
...
<h4 id="只能保证一个共享变量的原子操作" tabindex="-1"><a class="header-anchor" href="#只能保证一个共享变量的原子操作"><span>只能保证一个共享变量的原子操作</span></a></h4>
<p>CAS 只对单个共享变量有效，当操作涉及跨多个共享变量时 CAS 无效。但是从 JDK 1.5 开始，提供了<code v-pre>AtomicReference</code>类来保证引用对象之间的原子性，你可以把多个变量放在一个对象里来进行 CAS 操作.所以我们可以使用锁或者利用<code v-pre>AtomicReference</code>类把多个共享变量合并成一个共享变量来操作。</p>
<h2 id="synchronized-关键字" tabindex="-1"><a class="header-anchor" href="#synchronized-关键字"><span>synchronized 关键字</span></a></h2>
<h3 id="synchronized-是什么-有什么用" tabindex="-1"><a class="header-anchor" href="#synchronized-是什么-有什么用"><span>synchronized 是什么？有什么用？</span></a></h3>
<p><code v-pre>synchronized</code> 是 Java 中的一个关键字，翻译成中文是同步的意思，主要解决的是多个线程之间访问资源的同步性，可以保证被它修饰的方法或者代码块在任意时刻只能有一个线程执行。</p>
<p>在 Java 早期版本中，<code v-pre>synchronized</code> 属于 <strong>重量级锁</strong>，效率低下。这是因为监视器锁（monitor）是依赖于底层的操作系统的 <code v-pre>Mutex Lock</code> 来实现的，Java 的线程是映射到操作系统的原生线程之上的。如果要挂起或者唤醒一个线程，都需要操作系统帮忙完成，而操作系统实现线程之间的切换时需要从用户态转换到内核态，这个状态之间的转换需要相对比较长的时间，时间成本相对较高。</p>
<p>不过，在 Java 6 之后， <code v-pre>synchronized</code> 引入了大量的优化如自旋锁、适应性自旋锁、锁消除、锁粗化、偏向锁、轻量级锁等技术来减少锁操作的开销，这些优化让 <code v-pre>synchronized</code> 锁的效率提升了很多。因此， <code v-pre>synchronized</code> 还是可以在实际项目中使用的，像 JDK 源码、很多开源框架都大量使用了 <code v-pre>synchronized</code> 。</p>
...
<h3 id="如何使用-synchronized" tabindex="-1"><a class="header-anchor" href="#如何使用-synchronized"><span>如何使用 synchronized？</span></a></h3>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>2、修饰静态方法</strong> （锁当前类）</p>
...
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code><span class="token keyword">synchronized</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">method</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
    <span class="token comment">//业务代码</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>静态 <code v-pre>synchronized</code> 方法和非静态 <code v-pre>synchronized</code> 方法之间的调用互斥么？不互斥！如果一个线程 A 调用一个实例对象的非静态 <code v-pre>synchronized</code> 方法，而线程 B 需要调用这个实例对象所属类的静态 <code v-pre>synchronized</code> 方法，是允许的，不会发生互斥现象，因为访问静态 <code v-pre>synchronized</code> 方法占用的锁是当前类的锁，而访问非静态 <code v-pre>synchronized</code> 方法占用的锁是当前实例对象锁。</p>
<p><strong>3、修饰代码块</strong> （锁指定对象/类）</p>
<p>对括号里指定的对象/类加锁：</p>
<ul>
<li><code v-pre>synchronized(object)</code> 表示进入同步代码库前要获得 <strong>给定对象的锁</strong>。</li>
<li><code v-pre>synchronized(类.class)</code> 表示进入同步代码前要获得 <strong>给定 Class 的锁</strong></li>
</ul>
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code><span class="token keyword">synchronized</span><span class="token punctuation">(</span><span class="token keyword">this</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
    <span class="token comment">//业务代码</span>
<span class="token punctuation">}</span>
...
<h3 id="构造方法可以用-synchronized-修饰么" tabindex="-1"><a class="header-anchor" href="#构造方法可以用-synchronized-修饰么"><span>构造方法可以用 synchronized 修饰么？</span></a></h3>
<p>先说结论：<strong>构造方法不能使用 synchronized 关键字修饰。</strong></p>
<p>构造方法本身就属于线程安全的，不存在同步的构造方法一说。</p>
<h3 id="synchronized-底层原理了解吗" tabindex="-1"><a class="header-anchor" href="#synchronized-底层原理了解吗"><span>synchronized 底层原理了解吗？</span></a></h3>
<p>synchronized 关键字底层原理属于 JVM 层面的东西。</p>
<h4 id="synchronized-同步语句块的情况" tabindex="-1"><a class="header-anchor" href="#synchronized-同步语句块的情况"><span>synchronized 同步语句块的情况</span></a></h4>
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">SynchronizedDemo</span> <span class="token punctuation">{</span>
...
<p>上面的字节码中包含一个 <code v-pre>monitorenter</code> 指令以及两个 <code v-pre>monitorexit</code> 指令，这是为了保证锁在同步代码块代码正常执行以及出现异常的这两种情况下都能被正确释放。</p>
...
<h4 id="synchronized-修饰方法的的情况" tabindex="-1"><a class="header-anchor" href="#synchronized-修饰方法的的情况"><span>synchronized 修饰方法的的情况</span></a></h4>
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">SynchronizedDemo2</span> <span class="token punctuation">{</span>
...
<p>相关推荐：<a href="https://tech.youzan.com/javasuo-yu-xian-cheng-de-na-xie-shi/" target="_blank" rel="noopener noreferrer">Java 锁与线程的那些事 - 有赞技术团队<ExternalLinkIcon/></a> 。</p>
<p>🧗🏻 进阶一下：学有余力的小伙伴可以抽时间详细研究一下对象监视器 <code v-pre>monitor</code>。</p>
<h3 id="jdk1-6-之后的-synchronized-底层做了哪些优化-锁升级原理了解吗" tabindex="-1"><a class="header-anchor" href="#jdk1-6-之后的-synchronized-底层做了哪些优化-锁升级原理了解吗"><span>JDK1.6 之后的 synchronized 底层做了哪些优化？锁升级原理了解吗？</span></a></h3>
<p>在 Java 6 之后， <code v-pre>synchronized</code> 引入了大量的优化如自旋锁、适应性自旋锁、锁消除、锁粗化、偏向锁、轻量级锁等技术来减少锁操作的开销，这些优化让 <code v-pre>synchronized</code> 锁的效率提升了很多（JDK18 中，偏向锁已经被彻底废弃，前面已经提到过了）。</p>
<p>锁主要存在四种状态，依次是：无锁状态、偏向锁状态、轻量级锁状态、重量级锁状态，他们会随着竞争的激烈而逐渐升级。注意锁可以升级不可降级，这种策略是为了提高获得锁和释放锁的效率。</p>
<p><code v-pre>synchronized</code> 锁升级是一个比较复杂的过程，面试也很少问到，如果你想要详细了解的话，可以看看这篇文章：<a href="https://www.cnblogs.com/star95/p/17542850.html" target="_blank" rel="noopener noreferrer">浅析 synchronized 锁升级的原理与实现<ExternalLinkIcon/></a>。</p>
<h3 id="synchronized-和-volatile-有什么区别" tabindex="-1"><a class="header-anchor" href="#synchronized-和-volatile-有什么区别"><span>synchronized 和 volatile 有什么区别？</span></a></h3>
<p><code v-pre>synchronized</code> 关键字和 <code v-pre>volatile</code> 关键字是两个互补的存在，而不是对立的存在！</p>
<ul>
<li><code v-pre>volatile</code> 关键字是线程同步的轻量级实现，所以 <code v-pre>volatile</code>性能肯定比<code v-pre>synchronized</code>关键字要好 。但是 <code v-pre>volatile</code> 关键字只能用于变量而 <code v-pre>synchronized</code> 关键字可以修饰方法以及代码块 。</li>
<li><code v-pre>volatile</code> 关键字能保证数据的可见性，但不能保证数据的原子性。<code v-pre>synchronized</code> 关键字两者都能保证。</li>
...
<h3 id="reentrantlock-是什么" tabindex="-1"><a class="header-anchor" href="#reentrantlock-是什么"><span>ReentrantLock 是什么？</span></a></h3>
...
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code><span class="token comment">// 传入一个 boolean 值，true 时为公平锁，false 时为非公平锁</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>从上面的内容可以看出， <code v-pre>ReentrantLock</code> 的底层就是由 AQS 来实现的。关于 AQS 的相关内容推荐阅读 <a href="https://javaguide.cn/java/concurrent/aqs.html" target="_blank" rel="noopener noreferrer">AQS 详解<ExternalLinkIcon/></a> 这篇文章。</p>
<h3 id="公平锁和非公平锁有什么区别" tabindex="-1"><a class="header-anchor" href="#公平锁和非公平锁有什么区别"><span>公平锁和非公平锁有什么区别？</span></a></h3>
<ul>
...
<h3 id="synchronized-和-reentrantlock-有什么区别" tabindex="-1"><a class="header-anchor" href="#synchronized-和-reentrantlock-有什么区别"><span>synchronized 和 ReentrantLock 有什么区别？</span></a></h3>
<h4 id="两者都是可重入锁" tabindex="-1"><a class="header-anchor" href="#两者都是可重入锁"><span>两者都是可重入锁</span></a></h4>
...
<h4 id="synchronized-依赖于-jvm-而-reentrantlock-依赖于-api" tabindex="-1"><a class="header-anchor" href="#synchronized-依赖于-jvm-而-reentrantlock-依赖于-api"><span>synchronized 依赖于 JVM 而 ReentrantLock 依赖于 API</span></a></h4>
...
<h4 id="reentrantlock-比-synchronized-增加了一些高级功能" tabindex="-1"><a class="header-anchor" href="#reentrantlock-比-synchronized-增加了一些高级功能"><span>ReentrantLock 比 synchronized 增加了一些高级功能</span></a></h4>
...
<p>如果你想使用上述功能，那么选择 <code v-pre>ReentrantLock</code> 是一个不错的选择。</p>
<p>关于 <code v-pre>Condition</code>接口的补充：</p>
<blockquote>
<p><code v-pre>Condition</code>是 JDK1.5 之后才有的，它具有很好的灵活性，比如可以实现多路通知功能也就是在一个<code v-pre>Lock</code>对象中可以创建多个<code v-pre>Condition</code>实例（即对象监视器），<strong>线程对象可以注册在指定的<code v-pre>Condition</code>中，从而可以有选择性的进行线程通知，在调度线程上更加灵活。 在使用<code v-pre>notify()/notifyAll()</code>方法进行通知时，被通知的线程是由 JVM 选择的，用<code v-pre>ReentrantLock</code>类结合<code v-pre>Condition</code>实例可以实现“选择性通知”</strong> ，这个功能非常重要，而且是 <code v-pre>Condition</code> 接口默认提供的。而<code v-pre>synchronized</code>关键字就相当于整个 <code v-pre>Lock</code> 对象中只有一个<code v-pre>Condition</code>实例，所有的线程都注册在它一个身上。如果执行<code v-pre>notifyAll()</code>方法的话就会通知所有处于等待状态的线程，这样会造成很大的效率问题。而<code v-pre>Condition</code>实例的<code v-pre>signalAll()</code>方法，只会唤醒注册在该<code v-pre>Condition</code>实例中的所有等待线程。</p>
</blockquote>
<h3 id="可中断锁和不可中断锁有什么区别" tabindex="-1"><a class="header-anchor" href="#可中断锁和不可中断锁有什么区别"><span>可中断锁和不可中断锁有什么区别？</span></a></h3>
<ul>
...
<h2 id="reentrantreadwritelock" tabindex="-1"><a class="header-anchor" href="#reentrantreadwritelock"><span>ReentrantReadWriteLock</span></a></h2>
<p><code v-pre>ReentrantReadWriteLock</code> 在实际项目中使用的并不多，面试中也问的比较少，简单了解即可。JDK 1.8 引入了性能更好的读写锁 <code v-pre>StampedLock</code> 。</p>
<h3 id="reentrantreadwritelock-是什么" tabindex="-1"><a class="header-anchor" href="#reentrantreadwritelock-是什么"><span>ReentrantReadWriteLock 是什么？</span></a></h3>
<p><code v-pre>ReentrantReadWriteLock</code> 实现了 <code v-pre>ReadWriteLock</code> ，是一个可重入的读写锁，既可以保证多个线程同时读的效率，同时又可以保证有写入操作时的线程安全。</p>
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">ReentrantReadWriteLock</span>
...
<p><code v-pre>ReentrantReadWriteLock</code> 其实是两把锁，一把是 <code v-pre>WriteLock</code> (写锁)，一把是 <code v-pre>ReadLock</code>（读锁） 。读锁是共享锁，写锁是独占锁。读锁可以被同时读，可以同时被多个线程持有，而写锁最多只能同时被一个线程持有。</p>
<p>和 <code v-pre>ReentrantLock</code> 一样，<code v-pre>ReentrantReadWriteLock</code> 底层也是基于 AQS 实现的。</p>
...
<p><code v-pre>ReentrantReadWriteLock</code> 也支持公平锁和非公平锁，默认使用非公平锁，可以通过构造器来显示的指定。</p>
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code><span class="token comment">// 传入一个 boolean 值，true 时为公平锁，false 时为非公平锁</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="reentrantreadwritelock-适合什么场景" tabindex="-1"><a class="header-anchor" href="#reentrantreadwritelock-适合什么场景"><span>ReentrantReadWriteLock 适合什么场景？</span></a></h3>
<p>由于 <code v-pre>ReentrantReadWriteLock</code> 既可以保证多个线程同时读的效率，同时又可以保证有写入操作时的线程安全。因此，在读多写少的情况下，使用 <code v-pre>ReentrantReadWriteLock</code> 能够明显提升系统性能。</p>
<h3 id="共享锁和独占锁有什么区别" tabindex="-1"><a class="header-anchor" href="#共享锁和独占锁有什么区别"><span>共享锁和独占锁有什么区别？</span></a></h3>
<ul>
<li><strong>共享锁</strong>：一把锁可以被多个线程同时获得。</li>
<li><strong>独占锁</strong>：一把锁只能被一个线程获得。</li>
</ul>
<h3 id="线程持有读锁还能获取写锁吗" tabindex="-1"><a class="header-anchor" href="#线程持有读锁还能获取写锁吗"><span>线程持有读锁还能获取写锁吗？</span></a></h3>
<ul>
...
<p>读写锁的源码分析，推荐阅读 <a href="https://mp.weixin.qq.com/s/h3VIUyH9L0v14MrQJiiDbw" target="_blank" rel="noopener noreferrer">聊聊 Java 的几把 JVM 级锁 - 阿里巴巴中间件<ExternalLinkIcon/></a> 这篇文章，写的很不错。</p>
<h3 id="读锁为什么不能升级为写锁" tabindex="-1"><a class="header-anchor" href="#读锁为什么不能升级为写锁"><span>读锁为什么不能升级为写锁？</span></a></h3>
...
<h2 id="stampedlock" tabindex="-1"><a class="header-anchor" href="#stampedlock"><span>StampedLock</span></a></h2>
<p><code v-pre>StampedLock</code> 面试中问的比较少，不是很重要，简单了解即可。</p>
<h3 id="stampedlock-是什么" tabindex="-1"><a class="header-anchor" href="#stampedlock-是什么"><span>StampedLock 是什么？</span></a></h3>
<p><code v-pre>StampedLock</code> 是 JDK 1.8 引入的性能更好的读写锁，不可重入且不支持条件变量 <code v-pre>Condition</code>。</p>
<p>不同于一般的 <code v-pre>Lock</code> 类，<code v-pre>StampedLock</code> 并不是直接实现 <code v-pre>Lock</code>或 <code v-pre>ReadWriteLock</code>接口，而是基于 <strong>CLH 锁</strong> 独立实现的（AQS 也是基于这玩意）。</p>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>StampedLock</code> 提供了三种模式的读写控制模式：读锁、写锁和乐观读。</p>
<ul>
<li><strong>写锁</strong>：独占锁，一把锁只能被一个线程获得。当一个线程获取写锁后，其他请求读锁和写锁的线程必须等待。类似于 <code v-pre>ReentrantReadWriteLock</code> 的写锁，不过这里的写锁是不可重入的。</li>
<li><strong>读锁</strong> （悲观读）：共享锁，没有线程获取写锁的情况下，多个线程可以同时持有读锁。如果己经有线程持有写锁，则其他线程请求获取该读锁会被阻塞。类似于 <code v-pre>ReentrantReadWriteLock</code> 的读锁，不过这里的读锁是不可重入的。</li>
<li><strong>乐观读</strong>：允许多个线程获取乐观读以及读锁。同时允许一个写线程获取写锁。</li>
</ul>
<p>另外，<code v-pre>StampedLock</code> 还支持这三种锁在一定条件下进行相互转换 。</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>StampedLock</code> 在获取锁的时候会返回一个 long 型的数据戳，该数据戳用于稍后的锁释放参数，如果返回的数据戳为 0 则表示锁获取失败。当前线程持有了锁再次获取锁还是会返回一个新的数据戳，这也是<code v-pre>StampedLock</code>不可重入的原因。</p>
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code><span class="token comment">// 写锁</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="stampedlock-的性能为什么更好" tabindex="-1"><a class="header-anchor" href="#stampedlock-的性能为什么更好"><span>StampedLock 的性能为什么更好？</span></a></h3>
<p>相比于传统读写锁多出来的乐观读是<code v-pre>StampedLock</code>比 <code v-pre>ReadWriteLock</code> 性能更好的关键原因。<code v-pre>StampedLock</code> 的乐观读允许一个写线程获取写锁，所以不会导致所有写线程阻塞，也就是当读多写少的时候，写线程有机会获取写锁，减少了线程饥饿的问题，吞吐量大大提高。</p>
<h3 id="stampedlock-适合什么场景" tabindex="-1"><a class="header-anchor" href="#stampedlock-适合什么场景"><span>StampedLock 适合什么场景？</span></a></h3>
<p>和 <code v-pre>ReentrantReadWriteLock</code> 一样，<code v-pre>StampedLock</code> 同样适合读多写少的业务场景，可以作为 <code v-pre>ReentrantReadWriteLock</code>的替代品，性能更好。</p>
<p>不过，需要注意的是<code v-pre>StampedLock</code>不可重入，不支持条件变量 <code v-pre>Condition</code>，对中断操作支持也不友好（使用不当容易导致 CPU 飙升）。如果你需要用到 <code v-pre>ReentrantLock</code> 的一些高级性能，就不太建议使用 <code v-pre>StampedLock</code> 了。</p>
<p>另外，<code v-pre>StampedLock</code> 性能虽好，但使用起来相对比较麻烦，一旦使用不当，就会出现生产问题。强烈建议你在使用<code v-pre>StampedLock</code> 之前，看看 <a href="https://docs.oracle.com/javase/8/docs/api/java/util/concurrent/locks/StampedLock.html" target="_blank" rel="noopener noreferrer">StampedLock 官方文档中的案例<ExternalLinkIcon/></a>。</p>
<h3 id="stampedlock-的底层原理了解吗" tabindex="-1"><a class="header-anchor" href="#stampedlock-的底层原理了解吗"><span>StampedLock 的底层原理了解吗？</span></a></h3>
<p><code v-pre>StampedLock</code> 不是直接实现 <code v-pre>Lock</code>或 <code v-pre>ReadWriteLock</code>接口，而是基于 <strong>CLH 锁</strong> 实现的（AQS 也是基于这玩意），CLH 锁是对自旋锁的一种改良，是一种隐式的链表队列。<code v-pre>StampedLock</code> 通过 CLH 队列进行线程的管理，通过同步状态值 <code v-pre>state</code> 来表示锁的状态和类型。</p>
<p><code v-pre>StampedLock</code> 的原理和 AQS 原理比较类似，这里就不详细介绍了，感兴趣的可以看看下面这两篇文章：</p>
<ul>
<li><a href="https://javaguide.cn/java/concurrent/aqs.html" target="_blank" rel="noopener noreferrer">AQS 详解<ExternalLinkIcon/></a></li>
<li><a href="https://segmentfault.com/a/1190000015808032" target="_blank" rel="noopener noreferrer">StampedLock 底层原理分析<ExternalLinkIcon/></a></li>
</ul>
<p>如果你只是准备面试的话，建议多花点精力搞懂 AQS 原理即可，<code v-pre>StampedLock</code> 底层原理在面试中遇到的概率非常小。</p>
<h2 id="atomic-原子类" tabindex="-1"><a class="header-anchor" href="#atomic-原子类"><span>Atomic 原子类</span></a></h2>
<p>Atomic 原子类部分的内容我单独写了一篇文章来总结：<RouteLink to="/java-core/concurrent/atomic-classes.html">Atomic 原子类总结</RouteLink> 。</p>
<h2 id="参考" tabindex="-1"><a class="header-anchor" href="#参考"><span>参考</span></a></h2>
<ul>
<li>《深入理解 Java 虚拟机》</li>
<li>《实战 Java 高并发程序设计》</li>
<li>Guide to the Volatile Keyword in Java - Baeldung：<a href="https://www.baeldung.com/java-volatile" target="_blank" rel="noopener noreferrer">https://www.baeldung.com/java-volatile<ExternalLinkIcon/></a></li>
<li>不可不说的 Java“锁”事 - 美团技术团队：<a href="https://tech.meituan.com/2018/11/15/java-lock.html" target="_blank" rel="noopener noreferrer">https://tech.meituan.com/2018/11/15/java-lock.html<ExternalLinkIcon/></a></li>
<li>在 ReadWriteLock 类中读锁为什么不能升级为写锁？：<a href="https://cloud.tencent.com/developer/article/1176230" target="_blank" rel="noopener noreferrer">https://cloud.tencent.com/developer/article/1176230<ExternalLinkIcon/></a></li>
<li>高性能解决线程饥饿的利器 StampedLock：<a href="https://mp.weixin.qq.com/s/2Acujjr4BHIhlFsCLGwYSg" target="_blank" rel="noopener noreferrer">https://mp.weixin.qq.com/s/2Acujjr4BHIhlFsCLGwYSg<ExternalLinkIcon/></a></li>
<li>理解 Java 中的 ThreadLocal - 技术小黑屋：<a href="https://droidyue.com/blog/2016/03/13/learning-threadlocal-in-java/" target="_blank" rel="noopener noreferrer">https://droidyue.com/blog/2016/03/13/learning-threadlocal-in-java/<ExternalLinkIcon/></a></li>
<li>ThreadLocal (Java Platform SE 8 ) - Oracle Help Center：<a href="https://docs.oracle.com/javase/8/docs/api/java/lang/ThreadLocal.html" target="_blank" rel="noopener noreferrer">https://docs.oracle.com/javase/8/docs/api/java/lang/ThreadLocal.html<ExternalLinkIcon/></a></li>
</ul>
</div></template>
