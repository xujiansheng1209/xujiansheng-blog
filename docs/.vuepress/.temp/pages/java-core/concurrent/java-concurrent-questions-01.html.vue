<template><div><h2 id="什么是线程和进程" tabindex="-1"><a class="header-anchor" href="#什么是线程和进程"><span>什么是线程和进程?</span></a></h2>
<h3 id="何为进程" tabindex="-1"><a class="header-anchor" href="#何为进程"><span>何为进程?</span></a></h3>
...
<figure><img src="https://oss.javaguide.cn/github/javaguide/java/进程示例图片-Windows.png" alt="进程示例图片-Windows" tabindex="0"><figcaption>进程示例图片-Windows</figcaption></figure>
<h3 id="何为线程" tabindex="-1"><a class="header-anchor" href="#何为线程"><span>何为线程?</span></a></h3>
...
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">MultiThread</span> <span class="token punctuation">{</span>
...
<div class="language-plain line-numbers-mode" data-ext="plain" data-title="plain"><pre v-pre class="language-plain"><code>[5] Attach Listener //添加事件
...
<h2 id="java-线程和操作系统的线程有啥区别" tabindex="-1"><a class="header-anchor" href="#java-线程和操作系统的线程有啥区别"><span>Java 线程和操作系统的线程有啥区别？</span></a></h2>
...
<figure><img src="https://oss.javaguide.cn/github/javaguide/java/concurrent/three-types-of-thread-models.png" alt="常见的三种线程模型" tabindex="0"><figcaption>常见的三种线程模型</figcaption></figure>
<p>在 Windows 和 Linux 等主流操作系统中，Java 线程采用的是一对一的线程模型，也就是一个 Java 线程对应一个系统内核线程。Solaris 系统是一个特例（Solaris 系统本身就支持多对多的线程模型），HotSpot VM 在 Solaris 上支持多对多和一对一。具体可以参考 R 大的回答: <a href="https://www.zhihu.com/question/23096638/answer/29617153" target="_blank" rel="noopener noreferrer">JVM 中的线程模型是用户级的么？<ExternalLinkIcon/></a>。</p>
<p>虚拟线程在 JDK 21 顺利转正，关于虚拟线程、平台线程（也就是我们上面提到的 Java 线程）和内核线程三者的关系可以阅读我写的这篇文章：<RouteLink to="/java-core/new-features/java20.html">Java 20 新特性概览</RouteLink>。</p>
<h2 id="请简要描述线程与进程的关系-区别及优缺点" tabindex="-1"><a class="header-anchor" href="#请简要描述线程与进程的关系-区别及优缺点"><span>请简要描述线程与进程的关系,区别及优缺点？</span></a></h2>
<p>从 JVM 角度说进程和线程之间的关系。</p>
<h3 id="图解进程和线程的关系" tabindex="-1"><a class="header-anchor" href="#图解进程和线程的关系"><span>图解进程和线程的关系</span></a></h3>
...
<h3 id="程序计数器为什么是私有的" tabindex="-1"><a class="header-anchor" href="#程序计数器为什么是私有的"><span>程序计数器为什么是私有的?</span></a></h3>
<p>程序计数器主要有下面两个作用：</p>
<ol>
...
<h3 id="虚拟机栈和本地方法栈为什么是私有的" tabindex="-1"><a class="header-anchor" href="#虚拟机栈和本地方法栈为什么是私有的"><span>虚拟机栈和本地方法栈为什么是私有的?</span></a></h3>
<ul>
...
<h3 id="一句话简单了解堆和方法区" tabindex="-1"><a class="header-anchor" href="#一句话简单了解堆和方法区"><span>一句话简单了解堆和方法区</span></a></h3>
<p>堆和方法区是所有线程共享的资源，其中堆是进程中最大的一块内存，主要用于存放新创建的对象 (几乎所有对象都在这里分配内存)，方法区主要用于存放已被加载的类信息、常量、静态变量、即时编译器编译后的代码等数据。</p>
<h2 id="并发与并行的区别" tabindex="-1"><a class="header-anchor" href="#并发与并行的区别"><span>并发与并行的区别</span></a></h2>
<ul>
...
<h2 id="同步和异步的区别" tabindex="-1"><a class="header-anchor" href="#同步和异步的区别"><span>同步和异步的区别</span></a></h2>
<ul>
<li><strong>同步</strong>：发出一个调用之后，在没有得到结果之前， 该调用就不可以返回，一直等待。</li>
<li><strong>异步</strong>：调用在发出之后，不用等待返回结果，该调用直接返回。</li>
</ul>
<h2 id="为什么要使用多线程" tabindex="-1"><a class="header-anchor" href="#为什么要使用多线程"><span>为什么要使用多线程?</span></a></h2>
<p>先从总体上来说：</p>
<ul>
...
<h2 id="使用多线程可能带来什么问题" tabindex="-1"><a class="header-anchor" href="#使用多线程可能带来什么问题"><span>使用多线程可能带来什么问题?</span></a></h2>
<p>并发编程的目的就是为了能提高程序的执行效率进而提高程序的运行速度，但是并发编程并不总是能提高程序运行速度的，而且并发编程可能会遇到很多问题，比如：内存泄漏、死锁、线程不安全等等。</p>
<h2 id="如何理解线程安全和不安全" tabindex="-1"><a class="header-anchor" href="#如何理解线程安全和不安全"><span>如何理解线程安全和不安全？</span></a></h2>
...
<h2 id="单核-cpu-上运行多个线程效率一定会高吗" tabindex="-1"><a class="header-anchor" href="#单核-cpu-上运行多个线程效率一定会高吗"><span>单核 CPU 上运行多个线程效率一定会高吗？</span></a></h2>
<p>单核 CPU 同时运行多个线程的效率是否会高，取决于线程的类型和任务的性质。一般来说，有两种类型的线程：CPU 密集型和 IO 密集型。CPU 密集型的线程主要进行计算和逻辑处理，需要占用大量的 CPU 资源。IO 密集型的线程主要进行输入输出操作，如读写文件、网络通信等，需要等待 IO 设备的响应，而不占用太多的 CPU 资源。</p>
...
<h2 id="如何创建线程" tabindex="-1"><a class="header-anchor" href="#如何创建线程"><span>如何创建线程？</span></a></h2>
<p>一般来说，创建线程有很多种方式，例如继承<code v-pre>Thread</code>类、实现<code v-pre>Runnable</code>接口、实现<code v-pre>Callable</code>接口、使用线程池、使用<code v-pre>CompletableFuture</code>类等等。</p>
<p>不过，这些方式其实并没有真正创建出线程。准确点来说，这些都属于是在 Java 代码中使用多线程的方法。</p>
<p>严格来说，Java 就只有一种方式可以创建线程，那就是通过<code v-pre>new Thread().start()</code>创建。不管是哪种方式，最终还是依赖于<code v-pre>new Thread().start()</code>。</p>
<p>关于这个问题的详细分析可以查看这篇文章：<a href="https://mp.weixin.qq.com/s/NspUsyhEmKnJ-4OprRFp9g" target="_blank" rel="noopener noreferrer">大家都说 Java 有三种创建线程的方式！并发编程中的惊天骗局！<ExternalLinkIcon/></a>。</p>
<h2 id="说说线程的生命周期和状态" tabindex="-1"><a class="header-anchor" href="#说说线程的生命周期和状态"><span>说说线程的生命周期和状态?</span></a></h2>
<p>Java 线程在运行的生命周期中的指定时刻只可能处于下面 6 种不同状态的其中一个状态：</p>
<ul>
<li>NEW: 初始状态，线程被创建出来但没有被调用 <code v-pre>start()</code> 。</li>
<li>RUNNABLE: 运行状态，线程被调用了 <code v-pre>start()</code>等待运行的状态。</li>
...
<p>Java 线程状态变迁图(图源：<a href="https://mp.weixin.qq.com/s/UOrXql_LhOD8dhTq_EPI0w" target="_blank" rel="noopener noreferrer">挑错 |《Java 并发编程的艺术》中关于线程状态的三处错误<ExternalLinkIcon/></a>)：</p>
...
<blockquote>
<p>在操作系统层面，线程有 READY 和 RUNNING 状态；而在 JVM 层面，只能看到 RUNNABLE 状态（图源：<a href="https://howtodoinJava.com/" title="HowToDoInJava" target="_blank" rel="noopener noreferrer">HowToDoInJava<ExternalLinkIcon/></a>：<a href="https://howtodoinJava.com/Java/multi-threading/Java-thread-life-cycle-and-thread-states/" title="Java Thread Life Cycle and Thread States" target="_blank" rel="noopener noreferrer">Java Thread Life Cycle and Thread States<ExternalLinkIcon/></a>），所以 Java 系统一般将这两个状态统称为 <strong>RUNNABLE（运行中）</strong> 状态 。</p>
<p><strong>为什么 JVM 没有区分这两种状态呢？</strong> （摘自：<a href="https://www.zhihu.com/question/56494969/answer/154053599" target="_blank" rel="noopener noreferrer">Java 线程运行怎么有第六种状态？ - Dawell 的回答<ExternalLinkIcon/></a> ） 现在的时分（time-sharing）多任务（multi-task）操作系统架构通常都是用所谓的“时间分片（time quantum or time slice）”方式进行抢占式（preemptive）轮转调度（round-robin 式）。这个时间分片通常是很小的，一个线程一次最多只能在 CPU 上运行比如 10-20ms 的时间（此时处于 running 状态），也即大概只有 0.01 秒这一量级，时间片用后就要被切换下来放入调度队列的末尾等待再次调度。（也即回到 ready 状态）。线程切换的如此之快，区分这两种状态就没什么意义了。</p>
</blockquote>
<figure><img src="https://oss.javaguide.cn/github/javaguide/java/RUNNABLE-VS-RUNNING.png" alt="RUNNABLE-VS-RUNNING" tabindex="0"><figcaption>RUNNABLE-VS-RUNNING</figcaption></figure>
<ul>
<li>当线程执行 <code v-pre>wait()</code>方法之后，线程进入 <strong>WAITING（等待）</strong> 状态。进入等待状态的线程需要依靠其他线程的通知才能够返回到运行状态。</li>
<li><strong>TIMED_WAITING(超时等待)</strong> 状态相当于在等待状态的基础上增加了超时限制，比如通过 <code v-pre>sleep（long millis）</code>方法或 <code v-pre>wait（long millis）</code>方法可以将线程置于 TIMED_WAITING 状态。当超时时间结束后，线程将会返回到 RUNNABLE 状态。</li>
<li>当线程进入 <code v-pre>synchronized</code> 方法/块或者调用 <code v-pre>wait</code> 后（被 <code v-pre>notify</code>）重新进入 <code v-pre>synchronized</code> 方法/块，但是锁被其它线程占有，这个时候线程就会进入 <strong>BLOCKED（阻塞）</strong> 状态。</li>
<li>线程在执行完了 <code v-pre>run()</code>方法之后将会进入到 <strong>TERMINATED（终止）</strong> 状态。</li>
</ul>
<p>相关阅读：<a href="https://mp.weixin.qq.com/s/R5MrTsWvk9McFSQ7bS0W2w" target="_blank" rel="noopener noreferrer">线程的几种状态你真的了解么？<ExternalLinkIcon/></a> 。</p>
<h2 id="什么是线程上下文切换" tabindex="-1"><a class="header-anchor" href="#什么是线程上下文切换"><span>什么是线程上下文切换?</span></a></h2>
...
<h2 id="什么是线程死锁-如何避免死锁" tabindex="-1"><a class="header-anchor" href="#什么是线程死锁-如何避免死锁"><span>什么是线程死锁?如何避免死锁?</span></a></h2>
<h3 id="认识线程死锁" tabindex="-1"><a class="header-anchor" href="#认识线程死锁"><span>认识线程死锁</span></a></h3>
<p>线程死锁描述的是这样一种情况：多个线程同时被阻塞，它们中的一个或者全部都在等待某个资源被释放。由于线程被无限期地阻塞，因此程序不可能正常终止。</p>
<p>如下图所示，线程 A 持有资源 2，线程 B 持有资源 1，他们同时都想申请对方的资源，所以这两个线程就会互相等待而进入死锁状态。</p>
<figure><img src="https://oss.javaguide.cn/github/javaguide/java/2019-4死锁1.png" alt="线程死锁示意图 " tabindex="0"><figcaption>线程死锁示意图 </figcaption></figure>
<p>下面通过一个例子来说明线程死锁,代码模拟了上图的死锁的情况 (代码来源于《并发编程之美》)：</p>
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">DeadLockDemo</span> <span class="token punctuation">{</span>
...
<div class="language-plain line-numbers-mode" data-ext="plain" data-title="plain"><pre v-pre class="language-plain"><code>Thread[线程 1,5,main]get resource1
...
<h3 id="如何预防和避免线程死锁" tabindex="-1"><a class="header-anchor" href="#如何预防和避免线程死锁"><span>如何预防和避免线程死锁?</span></a></h3>
...
<blockquote>
<p><strong>安全状态</strong> 指的是系统能够按照某种线程推进顺序（P1、P2、P3……Pn）来为每个线程分配所需资源，直到满足每个线程对资源的最大需求，使每个线程都可顺利完成。称 <code v-pre>&lt;P1、P2、P3.....Pn&gt;</code> 序列为安全序列。</p>
</blockquote>
...
<h2 id="sleep-方法和-wait-方法对比" tabindex="-1"><a class="header-anchor" href="#sleep-方法和-wait-方法对比"><span>sleep() 方法和 wait() 方法对比</span></a></h2>
<p><strong>共同点</strong>：两者都可以暂停线程的执行。</p>
<p><strong>区别</strong>：</p>
<ul>
<li><strong><code v-pre>sleep()</code> 方法没有释放锁，而 <code v-pre>wait()</code> 方法释放了锁</strong> 。</li>
<li><code v-pre>wait()</code> 通常被用于线程间交互/通信，<code v-pre>sleep()</code>通常被用于暂停执行。</li>
<li><code v-pre>wait()</code> 方法被调用后，线程不会自动苏醒，需要别的线程调用同一个对象上的 <code v-pre>notify()</code>或者 <code v-pre>notifyAll()</code> 方法。<code v-pre>sleep()</code>方法执行完成后，线程会自动苏醒，或者也可以使用 <code v-pre>wait(long timeout)</code> 超时后线程会自动苏醒。</li>
<li><code v-pre>sleep()</code> 是 <code v-pre>Thread</code> 类的静态本地方法，<code v-pre>wait()</code> 则是 <code v-pre>Object</code> 类的本地方法。为什么这样设计呢？下一个问题就会聊到。</li>
</ul>
<h2 id="为什么-wait-方法不定义在-thread-中" tabindex="-1"><a class="header-anchor" href="#为什么-wait-方法不定义在-thread-中"><span>为什么 wait() 方法不定义在 Thread 中？</span></a></h2>
<p><code v-pre>wait()</code> 是让获得对象锁的线程实现等待，会自动释放当前线程占有的对象锁。每个对象（<code v-pre>Object</code>）都拥有对象锁，既然要释放当前线程占有的对象锁并让其进入 WAITING 状态，自然是要操作对应的对象（<code v-pre>Object</code>）而非当前的线程（<code v-pre>Thread</code>）。</p>
<p>类似的问题：<strong>为什么 <code v-pre>sleep()</code> 方法定义在 <code v-pre>Thread</code> 中？</strong></p>
<p>因为 <code v-pre>sleep()</code> 是让当前线程暂停执行，不涉及到对象类，也不需要获得对象锁。</p>
<h2 id="可以直接调用-thread-类的-run-方法吗" tabindex="-1"><a class="header-anchor" href="#可以直接调用-thread-类的-run-方法吗"><span>可以直接调用 Thread 类的 run 方法吗？</span></a></h2>
<p>这是另一个非常经典的 Java 多线程面试问题，而且在面试中会经常被问到。很简单，但是很多人都会答不上来！</p>
<p>new 一个 <code v-pre>Thread</code>，线程进入了新建状态。调用 <code v-pre>start()</code>方法，会启动一个线程并使线程进入了就绪状态，当分配到时间片后就可以开始运行了。 <code v-pre>start()</code> 会执行线程的相应准备工作，然后自动执行 <code v-pre>run()</code> 方法的内容，这是真正的多线程工作。 但是，直接执行 <code v-pre>run()</code> 方法，会把 <code v-pre>run()</code> 方法当成一个 main 线程下的普通方法去执行，并不会在某个线程中执行它，所以这并不是多线程工作。</p>
