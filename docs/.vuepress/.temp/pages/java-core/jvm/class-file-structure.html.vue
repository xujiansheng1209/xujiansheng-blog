<template><div><h2 id="回顾一下字节码" tabindex="-1"><a class="header-anchor" href="#回顾一下字节码"><span>回顾一下字节码</span></a></h2>
...
<p>Clojure（Lisp 语言的一种方言）、Groovy、Scala、JRuby、Kotlin 等语言都是运行在 Java 虚拟机之上。下图展示了不同的语言被不同的编译器编译成<code v-pre>.class</code>文件最终运行在 Java 虚拟机之上。<code v-pre>.class</code>文件的二进制格式可以使用 <a href="https://www.x-ways.net/winhex/" target="_blank" rel="noopener noreferrer">WinHex<ExternalLinkIcon/></a> 查看。</p>
...
<h2 id="class-文件结构总结" tabindex="-1"><a class="header-anchor" href="#class-文件结构总结"><span>Class 文件结构总结</span></a></h2>
<p>根据 Java 虚拟机规范，Class 文件通过 <code v-pre>ClassFile</code> 定义，有点类似 C 语言的结构体。</p>
<p><code v-pre>ClassFile</code> 的结构如下：</p>
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code><span class="token class-name">ClassFile</span> <span class="token punctuation">{</span>
    ...
    u2             interfaces_count<span class="token punctuation">;</span><span class="token comment">//接口数量</span>
    ...
    u2             fields_count<span class="token punctuation">;</span><span class="token comment">//字段数量</span>
    ...
    u2             methods_count<span class="token punctuation">;</span><span class="token comment">//方法数量</span>
...
<h3 id="魔数-magic-number" tabindex="-1"><a class="header-anchor" href="#魔数-magic-number"><span>魔数（Magic Number）</span></a></h3>
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code>    u4             magic<span class="token punctuation">;</span> <span class="token comment">//Class 文件的标志</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>每个 Class 文件的头 4 个字节称为魔数（Magic Number）,它的唯一作用是<strong>确定这个文件是否为一个能被虚拟机接收的 Class 文件</strong>。Java 规范规定魔数为固定值：0xCAFEBABE。如果读取的文件不是以这个魔数开头，Java 虚拟机将拒绝加载它。</p>
<h3 id="class-文件版本号-minor-major-version" tabindex="-1"><a class="header-anchor" href="#class-文件版本号-minor-major-version"><span>Class 文件版本号（Minor&amp;Major Version）</span></a></h3>
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code>    u2             minor_version<span class="token punctuation">;</span><span class="token comment">//Class 的小版本号</span>
    u2             major_version<span class="token punctuation">;</span><span class="token comment">//Class 的大版本号</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>紧接着魔数的四个字节存储的是 Class 文件的版本号：第 5 和第 6 个字节是<strong>次版本号</strong>，第 7 和第 8 个字节是<strong>主版本号</strong>。</p>
...
<h3 id="常量池-constant-pool" tabindex="-1"><a class="header-anchor" href="#常量池-constant-pool"><span>常量池（Constant Pool）</span></a></h3>
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code>    u2             constant_pool_count<span class="token punctuation">;</span><span class="token comment">//常量池的数量</span>
    cp_info        constant_pool<span class="token punctuation">[</span>constant_pool_count<span class="token operator">-</span><span class="token number">1</span><span class="token punctuation">]</span><span class="token punctuation">;</span><span class="token comment">//常量池</span>
...
<h3 id="访问标志-access-flags" tabindex="-1"><a class="header-anchor" href="#访问标志-access-flags"><span>访问标志(Access Flags)</span></a></h3>
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code>    u2             access_flags<span class="token punctuation">;</span><span class="token comment">//Class 的访问标记</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>在常量池结束之后，紧接着的两个字节代表访问标志，这个标志用于识别一些类或者接口层次的访问信息，包括：这个 Class 是类还是接口，是否为 <code v-pre>public</code> 或者 <code v-pre>abstract</code> 类型，如果是类的话是否声明为 <code v-pre>final</code> 等等。</p>
<p>类访问和属性修饰符:</p>
<figure><img src="https://oss.javaguide.cn/github/javaguide/java/访问标志.png" alt="类访问和属性修饰符" tabindex="0"><figcaption>类访问和属性修饰符</figcaption></figure>
<p>我们定义了一个 <code v-pre>Employee</code> 类</p>
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code><span class="token keyword">package</span> <span class="token namespace">top<span class="token punctuation">.</span>snailclimb<span class="token punctuation">.</span>bean</span><span class="token punctuation">;</span>
...
<figure><img src="https://oss.javaguide.cn/github/javaguide/java/查看类的访问标志.png" alt="查看类的访问标志" tabindex="0"><figcaption>查看类的访问标志</figcaption></figure>
<h3 id="当前类-this-class-、父类-super-class-、接口-interfaces-索引集合" tabindex="-1"><a class="header-anchor" href="#当前类-this-class-、父类-super-class-、接口-interfaces-索引集合"><span>当前类（This Class）、父类（Super Class）、接口（Interfaces）索引集合</span></a></h3>
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code>    u2             this_class<span class="token punctuation">;</span><span class="token comment">//当前类</span>
    u2             super_class<span class="token punctuation">;</span><span class="token comment">//父类</span>
    u2             interfaces_count<span class="token punctuation">;</span><span class="token comment">//接口数量</span>
    u2             interfaces<span class="token punctuation">[</span>interfaces_count<span class="token punctuation">]</span><span class="token punctuation">;</span><span class="token comment">//一个类可以实现多个接口</span>
...
<p>类索引用于确定这个类的全限定名，父类索引用于确定这个类的父类的全限定名，由于 Java 语言的单继承，所以父类索引只有一个，除了 <code v-pre>java.lang.Object</code> 之外，所有的 Java 类都有父类，因此除了 <code v-pre>java.lang.Object</code> 外，所有 Java 类的父类索引都不为 0。</p>
...
<h3 id="字段表集合-fields" tabindex="-1"><a class="header-anchor" href="#字段表集合-fields"><span>字段表集合（Fields）</span></a></h3>
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code>    u2             fields_count<span class="token punctuation">;</span><span class="token comment">//字段数量</span>
...
<figure><img src="https://oss.javaguide.cn/github/javaguide/java/字段表的结构.png" alt="字段表的结构 " tabindex="0"><figcaption>字段表的结构 </figcaption></figure>
<ul>
...
<h3 id="方法表集合-methods" tabindex="-1"><a class="header-anchor" href="#方法表集合-methods"><span>方法表集合（Methods）</span></a></h3>
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code>    u2             methods_count<span class="token punctuation">;</span><span class="token comment">//方法数量</span>
    method_info    methods<span class="token punctuation">[</span>methods_count<span class="token punctuation">]</span><span class="token punctuation">;</span><span class="token comment">//一个类可以有个多个方法</span>
...
<figure><img src="https://oss.javaguide.cn/github/javaguide/java/方法表的结构.png" alt="方法表的结构" tabindex="0"><figcaption>方法表的结构</figcaption></figure>
...
<h3 id="属性表集合-attributes" tabindex="-1"><a class="header-anchor" href="#属性表集合-attributes"><span>属性表集合（Attributes）</span></a></h3>
<div class="language-java line-numbers-mode" data-ext="java" data-title="java"><pre v-pre class="language-java"><code>   u2             attributes_count<span class="token punctuation">;</span><span class="token comment">//此类的属性表中的属性数</span>
   attribute_info attributes<span class="token punctuation">[</span>attributes_count<span class="token punctuation">]</span><span class="token punctuation">;</span><span class="token comment">//属性表集合</span>
...
<ul>
<li>《实战 Java 虚拟机》</li>
<li>Chapter 4. The class File Format - Java Virtual Machine Specification: <a href="https://docs.oracle.com/javase/specs/jvms/se8/html/jvms-4.html" target="_blank" rel="noopener noreferrer">https://docs.oracle.com/javase/specs/jvms/se8/html/jvms-4.html<ExternalLinkIcon/></a></li>
<li>实例分析 JAVA CLASS 的文件结构：<a href="https://coolshell.cn/articles/9229.html" target="_blank" rel="noopener noreferrer">https://coolshell.cn/articles/9229.html<ExternalLinkIcon/></a></li>
<li>《Java 虚拟机原理图解》 1.2.2、Class 文件中的常量池详解（上）：<a href="https://blog.csdn.net/luanlouis/article/details/39960815" target="_blank" rel="noopener noreferrer">https://blog.csdn.net/luanlouis/article/details/39960815<ExternalLinkIcon/></a></li>
</ul>
</div></template>
