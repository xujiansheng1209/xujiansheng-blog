...
<h2 id="什么是-springboot-自动装配" tabindex="-1"><a class="header-anchor" href="#什么是-springboot-自动装配"><span>什么是 SpringBoot 自动装配？</span></a></h2>
<p>我们现在提到自动装配的时候，一般会和 Spring Boot 联系在一起。但是，实际上 Spring Framework 早就实现了这个功能。Spring Boot 只是在其基础上，通过 SPI 的方式，做了进一步优化。</p>
<blockquote>
<p>SpringBoot 定义了一套接口规范，这套规范规定：SpringBoot 在启动时会扫描外部引用 jar 包中的<code v-pre>META-INF/spring.factories</code>文件，将文件中配置的类型信息加载到 Spring 容器（此处涉及到 JVM 类加载机制与 Spring 的容器知识），并执行类中定义的各种操作。对于外部 jar 来说，只需要按照 SpringBoot 定义的标准，就能将自己的功能装置进 SpringBoot。<br>
自 Spring Boot 3.0 开始，自动配置包的路径从<code v-pre>META-INF/spring.factories</code> 修改为 <code v-pre>META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports</code>。</p>
</blockquote>
...
<h2 id="springboot-是如何实现自动装配的" tabindex="-1"><a class="header-anchor" href="#springboot-是如何实现自动装配的"><span>SpringBoot 是如何实现自动装配的？</span></a></h2>
...
<li><code v-pre>@Configuration</code>：允许在上下文中注册额外的 bean 或导入其他配置类</li>
<li><code v-pre>@ComponentScan</code>：扫描被<code v-pre>@Component</code> (<code v-pre>@Service</code>,<code v-pre>@Controller</code>)注解的 bean，注解默认会扫描启动类所在的包下所有的类 ，可以自定义不扫描某些 bean。如下图所示，容器中将排除<code v-pre>TypeExcludeFilter</code>和<code v-pre>AutoConfigurationExcludeFilter</code>。</li>
</ul>
<figure><img src="https://oss.javaguide.cn/p3-juejin/bcc73490afbe4c6ba62acde6a94ffdfd~tplv-k3u1fbpfcp-watermark.png" alt="" tabindex="0"><figcaption></figcaption></figure>
<p><code v-pre>@EnableAutoConfiguration</code> 是实现自动装配的重要注解，我们以这个注解入手。</p>
<h3 id="enableautoconfiguration-实现自动装配的核心注解" tabindex="-1"><a class="header-anchor" href="#enableautoconfiguration-实现自动装配的核心注解"><span>@EnableAutoConfiguration:实现自动装配的核心注解</span></a></h3>
...
<h3 id="autoconfigurationimportselector-加载自动装配类" tabindex="-1"><a class="header-anchor" href="#autoconfigurationimportselector-加载自动装配类"><span>AutoConfigurationImportSelector:加载自动装配类</span></a></h3>
...
<p>该方法调用链如下：</p>
<figure><img src="https://oss.javaguide.cn/github/javaguide/system-design/framework/spring/3c1200712655443ca4b38500d615bb70~tplv-k3u1fbpfcp-watermark.png" alt="" tabindex="0"><figcaption></figcaption></figure>
...
<figure><img src="https://oss.javaguide.cn/p3-juejin/77aa6a3727ea4392870f5cccd09844ab~tplv-k3u1fbpfcp-watermark.png" alt="" tabindex="0"><figcaption></figcaption></figure>
...
<figure><img src="https://oss.javaguide.cn/p3-juejin/3d6ec93bbda1453aa08c52b49516c05a~tplv-k3u1fbpfcp-zoom-1.png" alt="" tabindex="0"><figcaption></figcaption></figure>
<p><strong>第 3 步</strong></p>
<p>获取需要自动装配的所有配置类，读取<code v-pre>META-INF/spring.factories</code></p>
<div class="language-plain line-numbers-mode" data-ext="plain" data-title="plain"><pre v-pre class="language-plain"><code>spring-boot/spring-boot-project/spring-boot-autoconfigure/src/main/resources/META-INF/spring.factories
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><figure><img src="https://oss.javaguide.cn/github/javaguide/system-design/framework/spring/58c51920efea4757aa1ec29c6d5f9e36~tplv-k3u1fbpfcp-watermark.png" alt="" tabindex="0"><figcaption></figcaption></figure>
<p>从下图可以看到这个文件的配置内容都被我们读取到了。<code v-pre>XXXAutoConfiguration</code>的作用就是按需加载组件。</p>
<figure><img src="https://oss.javaguide.cn/github/javaguide/system-design/framework/spring/94d6e1a060ac41db97043e1758789026~tplv-k3u1fbpfcp-watermark.png" alt="" tabindex="0"><figcaption></figcaption></figure>
...
<figure><img src="https://oss.javaguide.cn/github/javaguide/system-design/framework/spring/68fa66aeee474b0385f94d23bcfe1745~tplv-k3u1fbpfcp-watermark.png" alt="" tabindex="0"><figcaption></figcaption></figure>
...
<figure><img src="https://oss.javaguide.cn/github/javaguide/system-design/framework/spring/267f8231ae2e48d982154140af6437b0~tplv-k3u1fbpfcp-watermark.png" alt="" tabindex="0"><figcaption></figcaption></figure>
...
<h2 id="如何实现一个-starter" tabindex="-1"><a class="header-anchor" href="#如何实现一个-starter"><span>如何实现一个 Starter</span></a></h2>
<p>光说不练假把式，现在就来撸一个 starter，实现自定义线程池</p>
<p>第一步，创建<code v-pre>threadpool-spring-boot-starter</code>工程</p>
<figure><img src="https://oss.javaguide.cn/github/javaguide/system-design/framework/spring/1ff0ebe7844f40289eb60213af72c5a6~tplv-k3u1fbpfcp-watermark.png" alt="" tabindex="0"><figcaption></figcaption></figure>
<p>第二步，引入 Spring Boot 相关依赖</p>
<figure><img src="https://oss.javaguide.cn/github/javaguide/system-design/framework/spring/5e14254276604f87b261e5a80a354cc0~tplv-k3u1fbpfcp-watermark.png" alt="" tabindex="0"><figcaption></figcaption></figure>
<p>第三步，创建<code v-pre>ThreadPoolAutoConfiguration</code></p>
<figure><img src="https://oss.javaguide.cn/github/javaguide/system-design/framework/spring/1843f1d12c5649fba85fd7b4e4a59e39~tplv-k3u1fbpfcp-watermark.png" alt="" tabindex="0"><figcaption></figcaption></figure>
<p>第四步，在<code v-pre>threadpool-spring-boot-starter</code>工程的 resources 包下创建<code v-pre>META-INF/spring.factories</code>文件</p>
<figure><img src="https://oss.javaguide.cn/github/javaguide/system-design/framework/spring/97b738321f1542ea8140484d6aaf0728~tplv-k3u1fbpfcp-watermark.png" alt="" tabindex="0"><figcaption></figcaption></figure>
<p>最后新建工程引入<code v-pre>threadpool-spring-boot-starter</code></p>
<figure><img src="https://oss.javaguide.cn/github/javaguide/system-design/framework/spring/edcdd8595a024aba85b6bb20d0e3fed4~tplv-k3u1fbpfcp-watermark.png" alt="" tabindex="0"><figcaption></figcaption></figure>
<p>测试通过！！！</p>
<figure><img src="https://oss.javaguide.cn/github/javaguide/system-design/framework/spring/9a265eea4de742a6bbdbbaa75f437307~tplv-k3u1fbpfcp-watermark.png" alt="" tabindex="0"><figcaption></figcaption></figure>
