...
- 时间：2023.3~2024.3
- 职责（可选）：
  - 对生产数据中的核心数据表进行分析，结果显示表中 85%的数据为冷数据，对此对表进行了冷热数据的拆分，使得核心表的数据保持在 300 万左右，使得核心表的读写效率提高了 90%
  - 对扫描逻辑的代码流程重构，引入 Redis，将生产扫描的数据进行预热处理，引入了 Redisson 分布式锁，防止产品码同一时间的重复扫描。
  - 引入 SkyWalking 对分布式链路进行监控和追踪。
- 成绩【可选】
